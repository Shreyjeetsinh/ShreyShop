...
using ShreyShop.AppHost;
using Microsoft.Extensions.Configuration;

namespace ShreyShop.AppHost.UnitTests;
...
public class AppHostConfigurationTests
{
    ...
    private static string FindRepositoryRoot()
    {
        ...
        while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "ShreyShop.slnx")))
        {
            directory = directory.Parent;
        }

        return directory?.FullName
            ?? throw new DirectoryNotFoundException("Could not locate the ShreyShop repository root.");
    }
}
