...
using ShreyShop.ClientApp.Exceptions;

namespace ShreyShop.ClientApp.Services.RequestProvider;
...
public class RequestProvider(HttpMessageHandler _messageHandler) : IRequestProvider
{
    ...
    private static async Task<T?> ReadFromJsonAsync<T>(HttpContent content)
    {
        ...
        var data = await JsonSerializer.DeserializeAsync(contentStream, typeof(T), ShreyShopJsonSerializerContext.Default).ConfigureAwait(false);
        return (T?)data;
    }

    private static JsonContent SerializeToJson<T>(T data)
    {
        var typeInfo = ShreyShopJsonSerializerContext.Default.GetTypeInfo(typeof(T)) ?? throw new InvalidOperationException($"Missing type info for {typeof(T)}");
